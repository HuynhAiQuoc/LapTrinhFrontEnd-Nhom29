...
export class ChatAreaComponent implements OnInit, AfterViewChecked {
    ...
    this.webSocketService.responseServe().forEach(item => {
      if (JSON.parse(item).event === 'SEND_CHAT') {
        if ((JSON.parse(item).data.name === this.author) && (JSON.parse(item).data.type == 0)) {
          this.listMessage.push({ type: 'reply', name: JSON.parse(item).data.name, to: JSON.parse(item).data.to, mes: JSON.parse(item).data.mes, time: this.getTime() })
        }
      }
    });
  ...
  // request message to serve to create a new group chat
